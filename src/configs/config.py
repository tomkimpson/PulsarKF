...
canonical = {
    ...
    "noise_parameters":{
                         "process_noise": np.zeros(Npulsars,dtype=NF),      
                         "measurement_noise": NF(0.0) #standard deviation gaussian measurement noise.
            },


    "UKF_parameters": {"alpha": int(1),         # O'Leary uses 7e-4, Wan/Merwe recommend something generally small. e.g. 1e-3
                       "beta":  int(2),         # Beta incorporates prior knowledge of the distribution of x. For Guassians, beta=2 is optimal
                       "kappa": int(100),       # An extra scaling parameter. O'Leary uses 3-n_states, Wan/Merwe recommend =0
