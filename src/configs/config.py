...
canonical = {
    ...
    "noise_parameters":{
                         ...
                         "measurement_noise": NF(1e-2) #standard deviation gaussian measurement noise.
            },


    "UKF_parameters": {"alpha": 7e-4,         # O'Leary uses 7e-4, Wan/Merwe recommend something generally small. e.g. 1e-3
                       "beta":  int(2),         # Beta incorporates prior knowledge of the distribution of x. For Guassians, beta=2 is optimal
                       "kappa": 0,       # An extra scaling parameter. O'Leary uses 3-n_states, Wan/Merwe recommend =0
...
notebook = {
    ...
    "pulsar_parameters": {
         "f_psr" :           np.full((Npulsars,),100,dtype=NF),    
         "pulsar_distances": np.ones(Npulsars,dtype=NF), #every pulsar is 1kpc away
         "spindown_gamma":   np.full((Npulsars,),1e-20,dtype=NF),
         "spindown_n":       np.full((Npulsars,),3,dtype=NF),
         "pulsar_distribution": "NANOGrav", #one of random/uniform/orthogonal/NANOGrav. If NANOGrav all pulsar parameters are overriden with the true parameters of the NANOGrav pulsars
    ...
    "noise_parameters":{
                         "process_noise": NF(2e-12),    #standard deviation of process noise      
                         "measurement_noise": NF(1e-12) #standard deviation gaussian measurement noise.
            },
